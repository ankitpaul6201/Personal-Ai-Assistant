...
class JarvisLive:
        ...
        try:
            if name == "open_app":
                ...
                if self._vision_busy or (_now - self._vision_last_time) < _cooldown:
                    ...
                    if angle == "camera":
                        try:
                            if hasattr(self.ui, "get_latest_camera_frame") and self.ui.get_latest_camera_frame():
                                img_b = self.ui.get_latest_camera_frame()
                                mime_t = "image/jpeg"
                            else:
                            ...
                            print(f"[Vision] ❌ Camera capture error: {cam_err}")
                            self._vision_busy = False
                            return f"Camera capture unavailable ({cam_err}). Please check your webcam connection."
                    else:
                        try:
                            ...
                            print(f"[Vision] ❌ Screen capture error: {scr_err}")
                            self._vision_busy = False
                            return f"Screen capture error ({scr_err})."
    ...
    async def _send_startup_briefing(self) -> None:
        """
        Instant greeting based on time of day.
        """
        memory   = load_memory()
        ...
        p = (
            f"Greet the user by saying: '{greet} Sir, it is now {time_str}. What should I do for you, Sir?' "
            f"Do not fetch or state any news. Speak naturally. Do not call any tools.{lang_clause}{name_clause}"
        )
...
def main():
    face_file = resource_path("assets/images/face.png")
