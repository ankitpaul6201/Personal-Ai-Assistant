...
def _open_native(url: str, browser_name: Optional[str]) -> str:
    """
    Launches user's primary browser with native profile, signed-in accounts, and extensions.
    Does not attach automation overhead, avoiding empty blank tabs.
    ...
    if browser_name:
        ...
        # Uses native browser profile and settings.
...
class _BrowserSession:
    """
    Full session instance for a single browser.
    All browsers launch on native user profile with launch_persistent_context.
    """
    ...
    async def _adopt_page(self) -> Page:
        ...
        Reuses that tab instead of opening an extra about:blank page —
        prevents extra blank tabs from opening.
        """
    ...
    async def _launch(self):
        """
        Launches browser using native user profile.
...
def browser_control(
    ...
    # Opens requested site in native user browser.
    # Uses native browser profile and settings.
    # Uses native browser profile and settings.
    # Uses native browser profile and settings.
    # Uses native browser profile and settings.
