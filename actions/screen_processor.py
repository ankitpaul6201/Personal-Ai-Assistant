...
def _capture_camera() -> tuple[bytes, str]:
    ...
    if not cap or not cap.isOpened():
        raise RuntimeError("Webcam is unavailable or in use by another application.")
    ...
    ret, frame = cap.read()
    cap.release()
    time.sleep(0.15)  # Allow DirectShow hardware filter to safely release handle
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            if hasattr(self._player, "append_log"):
                                self._player.append_log(f"Jarvis: {full}")
                            elif hasattr(self._player, "_log") and hasattr(self._player._log, "append_log"):
                                self._player._log.append_log(f"Jarvis: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
